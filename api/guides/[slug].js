...
export default async function handler(request) {
  ...
  const places = await sbSelect(`view_guide_details?guide_slug=eq.${encodeURIComponent(slug)}&select=name,town,parish,category,image,ordering&order=ordering.asc&limit=3`);
  ...
  const image = cleanImage(guide.image_url) || places.map((place) => cleanImage(place.image)).find(Boolean) || null;
