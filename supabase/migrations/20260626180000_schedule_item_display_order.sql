-- Let promoters order lineup artists before set times are known.
...
create index if not exists event_schedule_items_lineup_order_idx
  on public.event_schedule_items (event_id, day_id, display_order, start_time);
