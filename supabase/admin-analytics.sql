-- ============================================================
-- TRODDR Admin — Overview + Analytics RPCs (Phase 1)
-- Idempotent. Safe to re-run.
--
-- Powers admin-overview.html and admin-analytics.html.
-- All access via admin token (public._is_admin), same contract
-- as get_admin_review_queue.
--
-- Analytics source of truth: public.analytics_events
--   place_viewed    entity_id = places.id (text)
--   event_viewed    entity_id = events.id (text)
--   special_viewed  entity_id = special slug; source_context->>'special_id' = specials.id
--   vendor_clicked  entity_id = events.id;   source_context->>'vendor_id' / 'vendor_name'
--   guide_viewed    entity_id = guide slug;  source_context->>'guide_id' = guides.id
--   search_performed source_context->>'query' / 'result_count'
-- "Active user" = distinct coalesce(user_id, 'anon:'||session_id).
-- Dates bucketed in America/Jamaica.
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- 1. Overview payload — one call for the whole Overview tab
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 2. Analytics payload — one call for the whole Analytics tab
-- ─────────────────────────────────────────────────────────────
...
grant execute on function public.admin_get_analytics(text, integer) to anon, authenticated;
